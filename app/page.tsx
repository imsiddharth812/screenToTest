...
function DashboardView({ user, logout }: { user: any, logout: () => void }) {
  ...
  const handleScenarioSuccess = (scenario: Scenario) => {
    ...
    const existingScenario = scenarios.find(s => s.id === scenario.id)
    ...
    if (existingScenario) {
      ...
      // Update in feature scenarios list
      if (selectedFeature) {
        setScenariosPerFeature(prev => ({
          ...prev,
          [selectedFeature.id]: (prev[selectedFeature.id] || []).map(s => s.id === scenario.id ? scenario : s)
        }))
      }
    } else {
      // Add new scenario
      ...
      if (selectedFeature) {
        setScenariosPerFeature(prev => ({
          ...prev,
          [selectedFeature.id]: [...(prev[selectedFeature.id] || []), scenario]
        }))
      }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                        ...
                        <div key={feature.id} className="feature-group">
                          ...
                          {expandedFeatures.has(feature.id) && (
                            <div className="ml-4">
                              ...
                              <div className="flex items-center px-2 py-1">
                                <button
                                  onClick={() => setCreateScenarioModal(true)}
        ...
        <div className="flex-1 overflow-hidden">
          {selectedScenario ? (
            <ScenarioTabs 
              ...
              onScenarioUpdate={handleScenarioSuccess}
            />
          ) : projects.length === 0 ? (
            /* Empty State */
      ...
      <CreateScenarioModal 
        isOpen={createScenarioModal}
        onClose={() => setCreateScenarioModal(false)}
